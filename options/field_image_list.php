<?php
$crb_tpl_dir = MAPIFY_PLUGIN_DIR . '/lib/carbon-fields/admin-templates';
?>
<table class="carbon-subcontainer <?php echo $container_tag_class_name . ' layout-' . $this->layout; ?>" data-min-values="<?php echo $this->get_min() ?>" data-max-values="<?php echo $this->get_max() ?>">
	...
	<tr class="carbon-row carbon-empty-row" style="display: none">
		<td colspan="2">
			<?php
			printf(__('No hay ninguna imagen aún. Haz click <a href="#">aquí</a> para agregar una.', 'crb'), $this->labels['plural_name']);
			?>
		</td>
	</tr>
		...
		<tr id="carbon-<?php echo $this->get_name() . '-' . $index; ?>-complex-container" class="carbon-row carbon-group-row"> <!-- .minimized -->
			...
			<td class="carbon-complex-entry-content">
				<?php if ( $this->layout == self::LAYOUT_TABLE ):
					...
					include($crb_tpl_dir . DIRECTORY_SEPARATOR . 'complex-button-holder.php');
					?>
	...
	<tr class="carbon-actions">
		<td colspan="<?php echo $this->layout == self::LAYOUT_TABLE ? '2' : '3'; ?>">
			<div class="carbon-button" style="display: none !important;">
				<a href="#" data-action="add" class="button">
					<?php printf(__('Agregar imagen', 'crb'), $this->labels['singular_name']); ?> <!-- Cambiar imagen por %s para revertir -->
			...
			<div class="carbon-button">
				<a href="#" data-action="add_multiple" class="button">
					<?php printf(__('Agregar imagenes', 'crb'), $this->labels['plural_name']); ?> <!-- Cambiar imagenes por %s para revertir -->
					<?php if(count($this->groups) > 1) echo '&#8681;'; ?>
				</a>
				<ul>
					<?php foreach ($this->groups as $group): ?>
