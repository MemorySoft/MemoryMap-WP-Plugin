...
!(function(window, document, $, undefined){
	...
	Fullscreener = function(element, options){
		...
		if (!this.$container.length) {
			...
			this.notify('¡No se ha encontrado el contenedor!');
			return;
	...
	Fullscreener.fn.calcRatios = function(){
		...
		if (!params.ratios.element) {
			...
			if (!$element.attr('width') || !$element.attr('height')) {
				...
				this.notify('¡La imagen debe tener los atributos "width" y "height"!');
				return;
			...
			if (!instance) {
				...
				if (typeof args == 'string') {
					...
					if (instance[args]) {
						...
						Fullscreener.fn.notify('¡No es un método válido!');
					}
